# Support fans that are disabled when temperature exceeds a set threshold
#
...
class TemperatureoffFan:
    def __init__(self, config):
        ...
        gcode.register_mux_command(
            "SET_TEMPERATUREOFF_FAN_TARGET", "TEMPERATUREOFF_FAN", self.name,
            self.cmd_SET_TEMPERATUREOFF_FAN_TARGET,
            desc=self.cmd_SET_TEMPERATUREOFF_FAN_TARGET_help)
    ...
    def get_status(self, eventtime):
        ...
        return status
    cmd_SET_TEMPERATUREOFF_FAN_TARGET_help = \
        "Sets a temperatureoff fan target and fan speed limits"
    def cmd_SET_TEMPERATUREOFF_FAN_TARGET(self, gcmd):
...
class ControlBangBang:
    def __init__(self, temperatureoff_fan, config):
        self.temperatureoff_fan = temperatureoff_fan
        self.max_delta = config.getfloat('max_delta', 2.0, above=0.)
        self.heating = False
    def temperature_callback(self, read_time, temp):
        current_temp, target_temp = self.temperatureoff_fan.get_temp(read_time)
        if (self.heating
...
class ControlPID:
    def __init__(self, temperatureoff_fan, config):
        ...
        imax = config.getfloat('pid_integral_max',
                               self.temperatureoff_fan.get_max_speed(), minval=0.)
    ...
    def temperature_callback(self, read_time, temp):
        ...
        bounded_co = max(0., min(self.temperatureoff_fan.get_min_speed(), co))
        self.temperatureoff_fan.set_speed(
            read_time, max(self.temperatureoff_fan.get_max_speed(),
                           self.temperatureoff_fan.get_min_speed() - bounded_co))
...
def load_config_prefix(config):
    return TemperatureoffFan(config)
